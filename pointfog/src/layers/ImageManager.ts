...
// import { P5Type, P5Image } from "colocorazon/dist/types/p5";
...
export default class ImageManager {
    ...
    loadAllImages(volume: Object, batchSize: number): void {
        ...
        for (let imageIx = 0; imageIx < nImages; imageIx++) {
            ...
            let imageURI = endpointStr + metaStr + coordStr + "/?no-cache=true";
            this.images[imageIx] = this.p.loadImage(imageURI, () => {
    ...
    draw(): void {
        if (this.readiness[this.currentZ]) {
            ...
            this.p.image(this.images[imageIx], this.position.x, this.position.y, this.imageWidth * this.scale, this.imageHeight * this.scale, 0, subIx * this.imageHeight, this.imageWidth, this.imageHeight);
        } else {
            ...
            this.p.rect(this.position.x, this.position.y, 500 * this.scale, 500 * this.scale);
