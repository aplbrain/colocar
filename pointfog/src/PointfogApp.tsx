...
export default class PointfogApp extends Component<any, any> {
    ...
    constructor(props: Object) {
        ...
        self.sketch = (p: P5Type) => {
            p.setup = function () {
                ...
                DB.getNextQuestion(
                    ...
                    .then((res: { question: Object; volume: Object }) => {
                        ...
                        self.artifactTags =
                            instructions.artifactTags || DEFAULT_ARTIFACT_TAGS;
                        ...
                        let emptyArtifacts = self.getEmptyArtifacts(
                            self.artifactTags
                        );
            ...
            p.mouseDragged = function () {
                if (p.mouseButton === p.RIGHT) {
                    ...
                    if (
                        self.layers.imageManager.imageCollision(
                            p.mouseX,
                            p.mouseY
                        )
                    ) {
                        ...
                        self.layers.imageManager.setPosition(
                            self.layers.imageManager.position.x - dX,
                            self.layers.imageManager.position.y - dY
                        );
                    }
                }
                self.updateUIStatus();
        ...
        this.handleMetadataModalClose = this.handleMetadataModalClose.bind(
            this
        );
        this.handleMetadataModalOpen = this.handleMetadataModalOpen.bind(this);

        this.handleArtifactReportClose = this.handleArtifactReportClose.bind(
            this
        );
        this.handleArtifactReportOpen = this.handleArtifactReportOpen.bind(
            this
        );
    ...
    */
        ...
        localForage
            .setItem(`pointfogStorage-${this.questionId}`, {
                artifacts,
                artifactImageUrls,
                nodes,
            })
    ...
    */
        ...
        let certain = confirm(
            "Attempting to submit. Are you sure that your data are ready?"
        );
        if (certain) {
            ...
            Promise.all([nodePromise, artifactPromise])
                .then((results) => {
    ...
    render() {
        ...
        let nodeCount = this.layers
            ? this.layers.pointcloudManager.getNodes().length
            : 0;
        let chipHTML = [];
        chipHTML.push(
            <div>
        ...
        if (this.volume && this.volume.bounds) {
            ...
            newX = Math.floor(
                oldX + xBounds[0] + (xBounds[1] - xBounds[0]) / 2
            );
            newY = Math.floor(
                oldY + yBounds[0] + (yBounds[1] - yBounds[0]) / 2
            );
            newZ = oldZ + zBounds[0];
        }
        ...
        if (this.artifactFlag) {
                            ...
                            onChange={(event: Object, checked: boolean) => {
                                ...
                                if (checked === true) {
                                    if (!(newZ in this.artifactImageUrls)) {
                                        this.artifactImageUrls[
                                            newZ
                                        ] = this.layers.imageManager.p.canvas.toDataURL(
                                            "image/jpeg",
                                            jpegQuality
                                        );
                                    }
            ...
            for (let zIndex in this.artifactImageUrls) {
                artifactSnapshots.push(
                    <tr key={`artifact_snapshot_${zIndex}`}>
                        <td style={{ padding: "2%" }}>
                            <img
                                alt="em-snapshot"
                        ...
                        <td style={{ padding: "2%" }}>
                            ...
                            <br />
                            {this.artifactTags
                                .filter((aTag) => this.artifacts[aTag][zIndex])
                                .join("/")}
                        </td>
                    </tr>
                );
            ...
            artifactSnapshotTitle.push(
                <tr key={`artifact_snapshot_title`}>
