...
const DEFAULT_ARTIFACT_TAGS = ["cracked", "dropped", "folded", "imaging", "stained"];
...
export default class PointfogApp extends Component<any, any> {
  ...
  constructor(props: Object) {
    ...
    self.sketch = (p: P5Type) => {
      p.setup = function () {
        ...
        DB.getNextQuestion(window.keycloak.profile.username, APP_NAMESPACE).then((res: {question: Object;volume: Object;}) => {
          if (!res || !res.question) {
          ...
          self.renderOrder = ["imageManager", "crosshairs", "pointcloudManager", "scrollbar"];
      ...
      p.keyPressed = function () {
        ...
        switch (p.keyCode) {
          ...
          case wKey:case upArrowKey:
            self.panDown();
            break;
          case sKey:case downArrowKey:
            self.panUp();
            break;
          case aKey:case leftArrowKey:
            self.panRight();
            break;
          case dKey:case rightArrowKey:
            self.panLeft();
            break;
          case qKey:
            self.decrementZ();
  ...
  insertStoredNodes() {
    ...
    localForage.getItem(`pointfogStorage-${this.questionId}`).then(storedData => {
  ...
  popBookmark(): void {
    ...
    let {
      ...
      y,
      z
    } = this.layers.pointcloudManager.popBookmark();
  ...
  saveNodes() {
    ...
    */
    ...
    localForage.setItem(`pointfogStorage-${this.questionId}`, {
      artifacts,
      artifactImageUrls,
      nodes
    }).then(() => {
    ...
    */
    ...
    if (certain) {
      ...
      let transformedNodes = nodes.map(oldNode => {
        ...
        let newX = oldNode.x + xBounds[0] + (xBounds[1] - xBounds[0]) / 2;
        let newY = oldNode.y + yBounds[0] + (yBounds[1] - yBounds[0]) / 2;
      ...
      let artifactPromise = DB.postArtifacts(this.questionId, this.artifacts);
  ...
  render() {
    ...
    chipHTML.push(<div>
                <div style={{ float: "right" }}>
                    <Tooltip title={prompt}>
                        <Chip style={{ margin: "0.5em 0" }} label={`${this.nodeType}: ${nodeCount}`} avatar={<Avatar style={{ backgroundColor: CHash(this.nodeType, 'hex') }}>{nodeKey}</Avatar>} />
                    </Tooltip>
                </div>
            </div>);
      ...
      chipHTML.push(<div>
                    <div style={{ float: "right" }}>
                        <Tooltip title={"Mark nodes as being low-confidence."}>
                            <Chip style={{ margin: "0.5em 0" }} label={"confidence"} avatar={<Avatar style={{ backgroundColor: "white" }}>{"C"}</Avatar>} />
                        </Tooltip>
                    </div>
                </div>);
    }
    ...
    if (this.volume && this.volume.bounds) {
      ...
      newX = Math.floor(oldX + xBounds[0] + (xBounds[1] - xBounds[0]) / 2);
      newY = Math.floor(oldY + yBounds[0] + (yBounds[1] - yBounds[0]) / 2);
      newZ = oldZ + zBounds[0];
    }
    ...
    if (this.artifactFlag) {
      ...
      for (let aIndex = 0; aIndex < this.artifactTags.length; aIndex++) {
        ...
        artifactChecklistHTML.push(<DialogContent key={`artifact_${artifact}`}>
                        <Checkbox checked={this.artifacts[artifact][newZ]} onChange={(event: Object, checked: boolean) => {
      ...
      for (let zIndex in this.artifactImageUrls) {
        artifactSnapshots.push(<tr key={`artifact_snapshot_${zIndex}`}>
                        <td style={{ "padding": "2%" }}>
                            <img alt="em-snapshot" src={this.artifactImageUrls[zIndex]} width="100%" />
                        </td>
                        <td style={{ "padding": "2%" }}>
                            z-index: {zIndex}
                            <br />
      ...
      artifactSnapshotTitle.push(<tr key={`artifact_snapshot_title`}>
    ...
    return <div>
            ...
            margin: "2em"
                            ...
                            <div style={{ float: "right", fontSize: "1.2em" }}>
                                <Chip style={{ margin: "0.5em 0" }} label={`x: ${xString}; y: ${yString}; z: ${zString}`} />
                            </div>
                            <br />
                            <div style={{ "float": "right" }}>
                                <div style={{ fontSize: "0.9em" }}>
                                    <Button style={{ opacity: 0.9 }} variant="fab" mini={true} onClick={this.handleSnackbarOpen}>
                                        <InfoIcon />
                                    </Button>
                                </div>
                                {this.artifactFlag ? <div style={{ fontSize: "0.9em" }}>
                                        <Button style={{ opacity: 0.9 }} color={artifactButtonColor} variant="fab" mini={true} onClick={this.handleMetadataModalOpen}>
                                            <FeedbackIcon />
                                        </Button>
                                    </div> : ""}
                                {this.artifactFlag ? <div style={{ fontSize: "0.9em" }}>
                                        <Button style={{ opacity: 0.9 }} variant="fab" mini={true} onClick={this.handleArtifactReportOpen}>
                                            <LibraryBooksIcon />
                                        </Button>
                                    </div> : ""}
                            </div>
                        ...
                        <Dialog id="artifact-annotation" open={this.state.artifactModalOpen} onClose={this.handleMetadataModalClose}>
                            <DialogTitle>
                        ...
                        <Dialog id="artifact-report" open={this.state.artifactReportOpen} onClose={this.handleArtifactReportClose}>
                            <table>
                                <tbody>
                        ...
                        <Snackbar open={this.state.snackbarOpen} onClose={this.handleSnackbarClose} ContentProps={{
            'aria-describedby': 'message-id'
          }} action={[<Button key="undo" color="secondary" size="small" onClick={this.handleSnackbarClose}>
                                    GOT IT
                                </Button>]} message={<div id="message-id">
                                <div>{this.prompt}</div>
                                <div>Task ID: {this.questionId}</div>
                            </div>} />

                        <Button variant="fab" color="secondary" style={STYLES["save"]} onClick={() => this.saveNodes()} disabled={this.state.saveInProgress}>
                            <SaveIcon />
                        </Button>
                        <Button variant="fab" color="primary" style={STYLES["submit"]} onClick={() => this.submitNodes()} disabled={this.state.saveInProgress}>
                            <SendIcon />
                        </Button>
                    </div>
